...
> **関連ドキュメント:** [先行研究・事例調査レポート](prior-research.md)
...
**議員への提供情報:**
- 第1読会の要約（議長作成）
- 自党以外の発言の要点（**匿名化**: モデル名・党名を伏せて提示 → [4.4 忖度防止・匿名化](#44-忖度防止匿名化メカニズム)）
- 議長が指定した「答えるべき質問」
...
- 反論の妥当性を評価
- 合意点と対立点を整理
- **少数派意見の保護:** 少数派の論拠を明示的に取り上げ、多数派に再検討を促す
- 修正協議が必要か判断
...
chair_powers:
  ...
  protect_minority: true            # 少数意見の保護義務

debate_rules:
  max_rounds: 3                     # 最大ラウンド数 (研究に基づき5→3に変更)
  min_rounds: 2                     # 最小ラウンド数 (所信表明+反論)
  parallel_speeches: true           # 並列発言を許可
  cross_party_context: true         # 他党の発言を共有する
  anonymize_in_rebuttal: true       # 反論ラウンドで発言元を匿名化
  require_rebuttal: true            # 反論の義務化 (最低1件)
...
# 対立度パラメータ (SocraSynth 由来)
# 各フェーズでの議論の対立度を制御
# temperature パラメータと連動: 高い対立度 = 高い temperature
contentiousness:
...
sycophancy_prevention:
  anti_conformity_prompt: true      # 反同調プロンプトを注入
  anonymize_sources: true           # 発言元の匿名化
  protect_minority_opinion: true    # 少数意見の保護
  measure_opinion_shift: true       # 意見変更の測定
```

### 4.4 忖度防止・匿名化メカニズム

先行研究により、マルチエージェント討論では以下の失敗モードが確認されている
（詳細は [先行研究レポート](prior-research.md) セクション3, 4 を参照）:

- **忖度 (Sycophancy)**: エージェントが他の意見に過度に同調する
- **多数派の暴政**: 正しい少数派が多数派に屈服する
- **集団思考**: 反対意見が抑制され、質の低い合意に至る
...
第2読会（反論ラウンド）では、他の議員の発言を匿名化して提示する。

```
匿名化前: 「革新党の議員A (GPT-5 nano) は MongoDB を推奨し...」
匿名化後: 「ある議員は MongoDB を推奨し...」
```

これにより、モデル名や党名に基づく知名度バイアス・アイデンティティバイアスを排除する
（Choi et al., 2025: 匿名化によりアイデンティティ駆動の忖度がほぼ完全に排除される）。

**採決時には匿名を解除**し、どの党・議員がどの投票を行ったかを議事録に記録する。
...
1. 第1読会後、多数派と少数派を識別
2. 第2読会で少数派の論拠を明示的に取り上げる
3. 多数派に対して「少数派の指摘に具体的に反論してください」と指示
4. 少数派の論拠が十分に検討されたかを議長が判断

#### 対策4: 対立度パラメータ (SocraSynth 由来)

議論の各フェーズで対立度を段階的に調整する。
対立度は API 呼び出し時の `temperature` パラメータに連動させる。

| フェーズ | 対立度 | temperature | 期待される効果 |
|---------|--------|-------------|---------------|
| 第1読会 | 0.8 | 0.9 | 大胆で独自性のある主張 |
| 第2読会 | 0.6 | 0.7 | 反論しつつ妥協点を探る |
| 修正協議 | 0.3 | 0.5 | 協調的に合意を形成 |

#### 対策5: 議論の価値測定

「討論 vs 投票」研究 (2025) の知見を踏まえ、議論が結果を改善しているかを測定:

- 第1読会終了時点の仮投票結果を記録
- 最終投票結果と比較
- 意見変更があった議員の変更理由を議事録に記録
- 各ラウンドでの論点の深化度を議長が評価し、議事録に付記
...
| 議員E | PostgreSQL |
| 議員F | MongoDB |
→ PostgreSQL 3, MongoDB 2, DynamoDB 1
...
| 議員 | 仮投票 | 最終投票 | 変更理由 |
|------|--------|---------|---------|
| 議員A | MongoDB | MongoDB | (変更なし) |
| 議員B | DynamoDB | MongoDB | 安定党の運用コスト指摘を受け、セルフホストの利点を再評価 |
| ... | | | |
...
- [ ] 党の綱領のカスタマイズ
- [ ] 忖度防止メカニズム (反同調プロンプト注入, 反論義務化)
- [ ] 反論ラウンドでの発言元匿名化
- [ ] 対立度パラメータ (contentiousness) の実装

### フェーズ3: 拡張機能
- [ ] 少数意見保護メカニズム
- [ ] 議論の価値測定 (仮投票 vs 最終投票の差分分析)
- [ ] 議事録の検索・参照
- [ ] 過去の議事録を踏まえた議論
